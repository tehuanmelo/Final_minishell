...
/* update_the_env_dir
# @brief: 
* Updates the cd directory in the enviornment containing both the OLDPWD and 
* the PWD path
# @example: 
...
{
	create_the_new_env_var(data, "OLDPWD", fetch_env_variable_char(data->env,
			"PWD"));
...
static bool	change_dir(t_data *data, char *path)
{
	...
	if (!ret)
	{
		error_msg_commad(COLOR_RED "cd: error in retrieving current directory",
			"getcwd: unable to access the parent directories" COLOR_RESET,
...
int	ft_cd_built_in(t_data *data, char **args)
{
	...
	if (strncmp(args[1], "-", 2) == 0)
	{
		...
		if (!path)
			return (error_msg_commad("cd", NULL, "OldPwd not set",
					EXIT_FAILURE));
		return (!change_dir(data, path));
	}
